...
def dice_groups(fn, ndice, outdir=None, overwrite=False):
    ...
    Return:
        fn_outlist: (str list) List of diced filenames, with path prepended
    '''
    # housekeeping
    if outdir == None:
    ...
    with fits.open(fn) as hdu1:
        ...
        print('Will produce %i output files, each with shape (%i, %i, %i, %i)' % (ndice, shape[0], ngroups_out, shape[2], shape[3]))
        ...
        for chunk in np.arange(ndice) + 1:
            ...
            # include chunk number out of total number of chunks in filename, e.g. chunk2_4_ (chunk 2 of 4)
            fnstr = '_%igroups_chunk%i_%i_uncal.fits' % (ngroups_out, chunk, ndice)
            fn_out = os.path.basename(fn).split('uncal.fits')[0] + fnstr
            fn_outlist.append(fn_out)
            if os.path.exists(os.path.join(outdir, fn_out)) and overwrite == False:
                print('%s already exists and you have chosen not to rewrite it. Continuing' % fn_out)
            else:
                hdu1[1].data = outdata
...
def slice_ints(fn, nslice, outdir=None, overwrite=False):
    ...
    with fits.open(fn) as hdu1:
        ...
        for chunk in np.arange(nslice) + 1:
            ...
            # update output header vals with new ngroups and updated timing values
            fn_out = os.path.basename(fn).split('uncal.fits')[0] + '_%iints_chunk%i_uncal.fits' % (nints_out, chunk)
            fn_outlist.append(fn_out)
            if os.path.exists(os.path.join(outdir, fn_out)) and overwrite == False:
                print('%s already exists and you have chosen not to rewrite it. Continuing' % fn_out)
            else:
...
def slice_ints_median(fn, nslice):
    '''
    Slice up input 3d calints files into chunks of integrations, discarding remainder ints, and calculate the
    median and standard deviation of the chunk.
    E.g. for a file with dimensions (65, 80, 80) and nslice=10,
    it will create 10 arrays, each with shape (6, 80, 80). The median of each of the 10 arrays is then taken
    to produce a list of 10 80x80 median images, and 10 80x80 standard deviation "images".

    Inputs:
        fn: (str) filename of uncal AMI file to slice up
        nslice: (int) number of chunks to divide ints into. if nints % nslice != 0, remove remainder ints.

    Return:
        med_list: (list of arrays) List of nslice arrays containing median of (nints//nslice) ints
        std_list: (list of arrays) List of nslice arrays containing standard deviation of the ints
    '''
    ...
    nints_out = shape[0] // nslice  # how many groups in each output file
    #print('Will produce %i output arrays, each with shape (%i, %i, %i)' % (
    #nslice, nints_out, shape[1], shape[2]))
    ...
    for chunk in np.arange(nslice) + 1:
        ...
        outdata = data[start:stop, :, :]
        print('\t Output data shape:', outdata.shape)
        med_out = np.median(outdata, axis=0)
        print("\t Median image shape:",med_out.shape)
        std_im = np.std(outdata, axis=0)
        med_list.append(med_out)
        std_list.append(std_im)
        # update the start value for the next chunk
        start = stop
    return med_list, std_list